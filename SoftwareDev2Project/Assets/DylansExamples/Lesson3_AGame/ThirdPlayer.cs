    ...
    void FixedUpdate()
    {
        if (moveUsingPhysics)
        {
            if (moveInWorldSpace)
            {
                ...
                rigid.AddForce(new Vector3(0, inputVector.x, 0) * forceAmount);
                rigid.AddTorque(inputVector.y * torqueAmount);
            }
            else if (moveInLocalSpace)
            {
                rigid.AddRelativeForce(new Vector3(0, inputVector.x, 0) * forceAmount);
                rigid.AddTorque(inputVector.y * torqueAmount);
            }
